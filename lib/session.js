...
CircularBuffer.prototype.get_tail = function () {
  return this.size > 0
    ? this.entries[(this.head + this.size - 1) % this.capacity]
    : undefined;
};
...
var Outgoing = function (connection) {
  ...
  this.deliveries = new CircularBuffer(20480);
  this.updated = [];
...
Outgoing.prototype.transfer_window = function () {
  if (this.remote_window) {
    return (
      this.remote_window -
      (this.next_transfer_id - this.remote_next_transfer_id)
    );
  } else {
    return 0;
  }
...
Outgoing.prototype.process = function () {
  ...
  while (this.next_pending_delivery < this.next_delivery_id) {
    ...
    if (d) {
      ...
      console.error(
        "ERROR: Next pending delivery not found: " + this.next_pending_delivery
      );
      break;
    }
  }
  ...
  for (var i = 0; i < this.updated.length; i++) {
    ...
    if (d.remote_state && d.remote_state.constructor.composite_type) {
      d.link.dispatch(
        d.remote_state.constructor.composite_type,
...
Incoming.prototype.on_transfer = function (frame, receiver) {
  ...
  if (receiver.is_remote_open()) {
    ...
    if (receiver._incomplete) {
      ...
      if (
        util.is_defined(frame.performative.delivery_id) &&
        current.id !== frame.performative.delivery_id
      ) {
        throw Error(
          "frame sequence error: delivery " +
            current.id +
            " not complete, got " +
            frame.performative.delivery_id
        );
      }
      data = Buffer.concat(
        [current.data, frame.payload],
      ...
      current = {
        ...
        state: undefined,
        remote_settled:
          frame.performative.settled === undefined
            ? false
            : frame.performative.settled,
    ...
    if (current.incomplete) {
      ...
      var msgctxt =
        current.format === 0
          ? { message: message.decode(data), delivery: current }
...
Session.prototype.attach_sender = function (args) {
  return attach(
    this.create_sender.bind(this),
    args,
    "target",
    this.get_option("sender_options", {})
  );
};
...
Session.prototype.attach_receiver = function (args) {
  return attach(
    this.create_receiver.bind(this),
    args,
    "source",
    this.get_option("receiver_options", {})
  );
};
...
Session.prototype._write_flow = function (link) {
  ...
  this.incoming.max_transfer_id =
    fields.next_incoming_id + fields.incoming_window;
  if (link) {
    ...
    if (error) {
      ...
      if (!handled) {
        EventEmitter.prototype.emit.call(
          ...
          "error",
          new SessionError(error.description, error.condition, this)
        );
      }
    } else {
...
Session.prototype.on_attach = function (frame) {
  ...
  if (!link) {
    ...
    link = frame.performative.role
      ? this.create_sender(name)
...
Session.prototype.on_disposition = function (frame) {
  if (frame.performative.role) {
    log.events(
      "[%s] Received disposition for outgoing transfers",
      this.connection.options.id
    );
    this.outgoing.on_disposition(frame.performative);
  } else {
    log.events(
      "[%s] Received disposition for incoming transfers",
      this.connection.options.id
    );
